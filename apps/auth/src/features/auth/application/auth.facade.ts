...
import { ValidateCommand, ValidateCommandHandler } from './commands/validate';
...
export class AuthFacade {
  ...
  commands = {
    ...
    validate: (accessToken: string) => this.validate(accessToken),
  };
  ...
  private validate(accessToken: string) {
    return this.commandBus.execute<
      ValidateCommand,
      Awaited<ReturnType<ValidateCommandHandler['execute']>>
    >(new ValidateCommand(accessToken));
  }
}
